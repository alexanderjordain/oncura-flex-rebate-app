...
new_rads_sf = rc4.number_input(
    "Rads — self-funded", value=float(rates.get("rads_self_funded", 0.04)),
...
# Audit manifest + ledger summary + Clear-ledger danger zone have moved to the
# dedicated **Audit & Tracking** page (`pages/audit_log.py`). Settings is now
# scoped to operational config edits (rates, routing, backup/restore) only.
st.info(
    ":material/inventory_2: Looking for the audit manifest, the processed-payments ledger "
...
with st.expander(":material/outgoing_mail: Misc. Reports", expanded=False):
    ...
    if _wol:
        ...
        # OPD cross-check status. When OPD is unreachable the list fails open:
        # every clinic is kept and flagged UNVERIFIED until OPD is reachable again.
        if _wol.get("opd_error"):
            st.warning(
                "OPD cross-check unavailable, so already-trained clinics could not be "
                "filtered out. Every clinic is shown and flagged UNVERIFIED until OPD is "
                f"reachable again. ({_wol['opd_error']})", icon=":material/link_off:")
...
# Danger zone (Clear ledger) has moved to the Audit & Tracking page.
